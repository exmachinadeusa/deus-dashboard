...
import { log, db, redis } from "../index.js";
import type { DeusContext } from "../index.js";
import { checkCommandRules } from "./rules.js";
import { detectIntent, type IntentResult } from "./ollama.js";
import { callClaude, callClaudeVision, MODEL_HAIKU } from "./router.js";
...
export async function smartDispatch(
  ...
  try {
    ...
    if (commandMatch?.matched) {
      ...
      log.info(
        { command: commandMatch.command, latencyMs: latency },
        "✅ Tier 1: Kural eşleşti"
      );
    ...
    if (ollamaResult && ollamaResult.intent && ollamaResult.intent !== 'unknown' && ollamaResult.confidence > 0.6) {
      const latency = Date.now() - startTime;
      log.info(
        { intent: ollamaResult.intent, confidence: ollamaResult.confidence, latencyMs: latency },
        "✅ Tier 2: Ollama confident"
      );
    ...
    if (ctx.message?.photo && ctx.message.photo.length > 0) {
      ...
      try {
        ...
        const mediaType: "image/jpeg" | "image/png" | "image/webp" | "image/gif" = "image/jpeg"; // Telegram photos are JPEG
...
`;
        ...
        log.info(
          { latencyMs: latency },
          "✅ Tier 3: Claude Vision dekont parse"
        );
    ...
    log.info(
      { latencyMs: latency },
      "⚠️ Tier 4: Claude Haiku fallback"
    );
 ...
 */
export async function logDispatchMetrics(
  userId: number,
