...
export type RuleVerdict =
  | "pass"          // Kural geçti, devam et
  | "reject"        // Kesin red, AI'a gerek yok
  | "auto_approve"  // Kesin onay, AI'a gerek yok
  | "needs_ai"      // Bu kural karar veremedi, AI'a yolla
...
export function validateIban(iban: string | null): RuleResult {
  if (!iban) {
...
export async function checkDuplication(receiptNumber: string | null): Promise<RuleResult> {
  if (!receiptNumber) {
  ...
  if (data) {
    await redis.setex(cacheKey, 86400, data["id"] as string);
    return {
      verdict: "reject",
...
export async function checkBlacklist(values: {
  ...
  if (values.iban) checks.push({ type: "iban", value: values.iban.replace(/\s/g, "") });
  if (values.name) checks.push({ type: "name", value: values.name.toLowerCase().trim() });
  if (values.tc) checks.push({ type: "tc", value: values.tc.trim() });
  if (values.phone) checks.push({ type: "phone", value: values.phone.replace(/\D/g, "") });
  ...
  await Promise.all(
    checks.map((c) => redis.setex(`bl:${c.type}:${c.value}:clean`, 1800, "1"))
  );
...
export async function findDepartmentByIban(receiverIban: string | null): Promise<RuleResult> {
  if (!receiverIban) {
  ...
  const cached = await redis.get(cacheKey);
  if (cached) {
    const dept = JSON.parse(cached) as Record<string, unknown>;
    return {
      verdict: "pass",
      reason: `Departman bulundu (cache): ${dept["name"] as string}`,
  ...
  return {
    verdict: "pass",
    reason: `Departman: ${data["name"] as string}`,
...
export function checkDepartmentLimit(
  amount: number,
...
// ── TAM DEKONT KURAL ZİNCİRİ ─────────────────────────────────

export interface RuleChainResult {
  finalVerdict: RuleVerdict;
  rules: Array<{ name: string; result: RuleResult }>;
  department?: Record<string, unknown>;
  aiRequired: boolean;
}

export async function runReceiptRuleChain(
  receipt: ReceiptData,
  memberName: string | null,
  expectedAmount?: number
): Promise<RuleChainResult> {
  const rules: Array<{ name: string; result: RuleResult }> = [];

  const run = (name: string, result: RuleResult): boolean => {
  ...
  const department = deptResult.data?.["department"] as Record<string, unknown> | undefined;

  if (department && receipt.amount) {
    const limitResult = checkDepartmentLimit(
      receipt.amount,
      department["current_balance"] as number,
      department["daily_limit"] as number
    );
