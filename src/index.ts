...
bot.command("ogret", async (ctx) => {
  ...
  if (args.length < 2) {
    await ctx.reply(
      "📚 *Kullanım:*\n`/ogret <eskalasyon_id> <cevap>`\n\nÖrnek:\n`/ogret esc_abc123 Yatırım 1-3 iş günü içinde işlenir.`",
      { parse_mode: "Markdown" }
    );
    return;
  }
  ...
  try {
    ...
    await ctx.reply(
      `✅ *Öğretildi!*\n\n` +
        ...
        `DEUS bir sonraki soruda bunu hatırlayacak.`,
      { parse_mode: "Markdown" }
    );
...
bot.on("message:photo", async (ctx) => {
  await ctx.reply(
    "📄 Dekont alındı. İşleniyor...\n_(Vision analizi başlatıldı)_",
    { parse_mode: "Markdown" }
  );
...
bot.on("message:text", async (ctx) => {
  ...
  try {
    ...
    if (result.strategy === "auto" && result.reply) {
      ...
      await ctx.reply(
        result.reply ?? "Bu konuyu yetkililere iletiyorum, kısa sürede dönüş yapılacak."
      );
