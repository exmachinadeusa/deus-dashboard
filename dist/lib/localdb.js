...
export function insertTransaction(txn) {
  ...
  `);
    try {
        stmt.run(txn.id, txn.customer_id, txn.reference_id, txn.transaction_type, txn.amount, txn.currency, txn.status, txn.approval_level, txn.approved_by, txn.metadata);
        return true;
    }
    catch (err) {
...
export function updateWalletBalance(customerId, amount) {
    const db = getDB();
