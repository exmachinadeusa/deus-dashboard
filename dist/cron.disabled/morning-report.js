...
async function generateDailyReconciliation() {
    ...
    for (const siteId of Object.keys(bysite)) {
        ...
        // Calculate totals
        const deposits = siteTxns.filter((t) => t.transaction_type === "deposit" && t.status === "completed");
        const withdrawals = siteTxns.filter((t) => t.transaction_type === "withdrawal" && t.status === "completed");
