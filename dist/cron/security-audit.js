...
async function checkRapidWithdrawals() {
    ...
    return Object.values(grouped).filter((g) => g.count >= THRESHOLDS.RAPID_WITHDRAW_COUNT);
}
...
async function checkTransactionSuccessRate() {
    ...
    const failed = data.filter((t) => t.status === "failed" || t.status === "rejected").length;
...
async function runSecurityAudit() {
    ...
    const alarms = [];
    try {
        const [highTx, rapidWithdraws, pendingEsc, txStats, health, kasaSummary] = await Promise.all([
        ...
        if (highTx.count > 0) {
            alarms.push(`⚠️ *Yüksek Miktar:* ${highTx.count} işlem (max: ${highTx.maxAmount.toLocaleString("tr-TR")} ₺)`);
        }
        ...
        if (txStats.rate > THRESHOLDS.FAILED_TX_RATE && txStats.total > 10) {
            alarms.push(`❌ *Yüksek başarısızlık oranı:* %${Math.round(txStats.rate * 100)} (${txStats.failed}/${txStats.total})`);
        }
        ...
        const kasaLines = kasaSummary.length === 0
            ? "Veri yok"
            : kasaSummary
                .map((k) => `• ${k.site_id}: +${(k.total_deposit ?? 0).toLocaleString("tr-TR")} / -${(k.total_withdrawal ?? 0).toLocaleString("tr-TR")} = ${(k.net_balance ?? 0).toLocaleString("tr-TR")} ₺`)
                .join("\n");
        const report = `${statusIcon} *DEUS Gece Güvenlik Raporu*\n` +
        ...
        await sendTelegram(`❌ *Güvenlik Denetimi Hatası*\n\`${String(err)}\``).catch(() => { });
        process.exit(1);
    }
}
