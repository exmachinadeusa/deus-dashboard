...
async function analyzePatterns(escalations) {
    if (escalations.length === 0)
        return "Eskalasyon bulunamadı.";
...
...`,
        ...
        if (!response.ok)
            throw new Error("Ollama yanıt vermedi");
...
[
  ...
  ...
        ...
        log.info({ input: data.usage.input_tokens, output: data.usage.output_tokens }, "Sonnet haftalık öğrenme token kullanımı");
...
async function applyKBImprovements(lowPerforming, improvements) {
    let updatedCount = 0;
    for (const improvement of improvements) {
        const original = lowPerforming.find((e) => e.question === improvement.question);
        if (!original)
            continue;
...
async function runWeeklyLearning() {
    ...
    try {
        ...
        log.info({ total: escalations.length, unresolved: unresolvedCount, lowKB: lowPerforming.length }, "Haftalık veri toplandı");
        const patterns = await analyzePatterns(escalations.map((e) => ({ question: e.question, category: e.category ?? "general" })));
        ...
        const report = `🧠 *DEUS Haftalık Öğrenme Raporu*\n` +
        ...
        await sendTelegramMessage(`❌ *Haftalık Öğrenme Hatası*\n\`${String(err)}\``).catch(() => { });
        process.exit(1);
    }
}
