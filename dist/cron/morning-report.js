...
async function sendMorningReport() {
    ...
    if (txErr)
        console.error("transactions_v2 fetch hatası:", txErr.message);
    ...
    const { count: kbCount } = await supabase
        ...
        .select("id", { count: "exact" })
        .gte("created_at", `${yesterday}T00:00:00`);
    const formatTry = (n) => `₺${n.toLocaleString("tr-TR", { minimumFractionDigits: 2 })}`;
    const report = `🌅 *DEUS — Günlük Rapor (${today})*\n` +
        `━━━━━━━━━━━━━━━━━━\n` +
        `📅 Dün (${yesterday}) özeti:\n\n` +
