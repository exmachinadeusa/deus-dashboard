...
let _bot = null;
export function initNotify(bot) {
    _bot = bot;
}
function bot() {
    if (!_bot)
        throw new Error("Bot henüz init edilmedi");
    return _bot;
}
// ── PARA FORMAT ───────────────────────────────────────────────
...
// ── ÜYEYİ BİLDİR ─────────────────────────────────────────────
export async function notifyMember(memberTgId, result) {
    let message = "";
...
export async function notifyAdmin(opts) {
    ...
    try {
        await bot().api.sendMessage(adminChatId, `${prefix} *${opts.title}*\n\n${opts.message}`, { parse_mode: "Markdown" });
    }
    catch (err) {
...
export async function handleApprovalCallback(callbackData, operatorTgId, operatorName) {
...
export function formatKasaMessage(opts) {
    const closing = opts.openingBalance +
        opts.deposits -
        opts.withdrawals -
