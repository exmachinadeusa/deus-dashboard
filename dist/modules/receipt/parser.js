...
import { runReceiptRuleChain, } from "../../ai/rules.js";
const anthropic = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY });
// ── FOTOĞRAFTAN PARSE ─────────────────────────────────────────
export async function parseReceiptFromPhoto(fileUrl) {
                            ...
                            text: `Bu dekont görselini analiz et ve şu JSON formatında döndür:
{
  ...
  "confidence": 0.95
        ...
        log.info({ confidence: parsed.confidence, amount: parsed.amount, senderName: parsed.senderName }, "Dekont parse edildi");
        return parsed;
    }
    catch (err) {
...
// ── METİNDEN PARSE ───────────────────────────────────────────
export async function parseReceiptFromText(text) {
...
export async function processReceipt(opts) {
    ...
    log.info({ verdict: ruleResult.finalVerdict, rules: ruleResult.rules.length }, "Kural zinciri tamamlandı");
    ...
    const dept = ruleResult.department;
